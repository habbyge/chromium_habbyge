...
class ASH_EXPORT ContentsView : public views::View,
 ...
 public:
  ...
  AppListView* app_list_view() const {
    return app_list_view_;
  }

  ExpandArrowView* expand_arrow_view() const {
 ...
 private:
  ...
  void AddLauncherPageInternal(std::unique_ptr<AppListPage> view, AppListState state);
  ...
  // bounds.
  int GetSearchBoxTopForViewState(AppListState state, AppListViewState view_state) const;
