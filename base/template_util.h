...
template <typename T, typename = void> // 1/2 首选写TMP的泛化版本
struct SupportsOstreamOperator : std::false_type {};
template <typename T> // 2/2 再写TMP的特化版本
// 在std::declval<std::ostream&>中默默使用了 “引用折叠规则”，这里其实使用 std::void_t<> 更好
...
template <typename T, typename = void> // 这里 void 相当于 std::void_t<>
...
// 在重载集中表达偏好的助手。如果给定的一组参数有多个重载可用，则将选择优先级较高的重载。
template <size_t I>
struct priority_tag : priority_tag<I - 1> {}; // 递归
